...
public abstract class AbstractMP3Object extends java.lang.Object {
    ...
    protected AbstractMP3FragmentBody relateBody;
    ...
    public void setRelateBody(AbstractMP3FragmentBody relateBody){
    	this.relateBody = relateBody;
    }
    ...
    public void readByteArray(final byte[] arr, final int offset) {
    	if(this.relateBody instanceof FrameBodyAPIC){
    		readString(new String(getStringByte(arr,offset)), 0);
    	}else{
    		String enc = this.relateBody.getTextEnconding();
    		try {
    			readString(new String(getStringByte(arr,offset),enc), 0);
    ...
    private byte[] getStringByte(byte[] arr,int offset){
    	byte[] returnStrByte = new byte[arr.length-offset];
    	for (int i = 0; i < returnStrByte.length; i++) {
    		returnStrByte[i] = arr[i+offset];
		}
    	return returnStrByte;
    }
    ...
    public byte[] writeByteArray() {
        try {
			return writeString().getBytes(this.relateBody.getTextEnconding());
		} catch (UnsupportedEncodingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
